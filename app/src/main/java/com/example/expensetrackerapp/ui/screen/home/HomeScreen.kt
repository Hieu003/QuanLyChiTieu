...
fun SummaryBox(tabIndex:Int, expenseList: State<List<Expense>>){
    ...
    Column(
        ...
        Text(
            text = "Tổng chi tiêu trong ${TabsMap[tabIndex]!!}",
            color = Color.White,
            fontSize = 16.sp,
            fontWeight = FontWeight.Medium
        )
        Text(
            text = " ${ConvertDecimal(expenseListSummary)} VND",
            color = Color.White,
            fontSize = 40.sp,
            fontWeight = FontWeight.Medium,
        )
...
fun ListSummary(viewModel:MainViewModel, expenseList: State<List<Expense>>){
    Column{
        Text(
            text = "Hôm nay , ${DateConverter(System.currentTimeMillis())}",
            fontSize = 20.sp,
            fontWeight = FontWeight.Medium
        )
...
fun CardItem(expense: Expense,viewModel: MainViewModel){
    ...
    Card(
        ...
        Row(
            ...
            Text(
                text = " ${ConvertDecimal(expense.amount)} VND",
                fontWeight = FontWeight.Medium
            )
        }
